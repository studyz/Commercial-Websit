...
class Digitalworld_Widgets_Instagram extends WP_Widget {
	function __construct() {
		$widget_ops  = array(
			'description' => esc_html__( 'Show off your favorite Instagram photos', 'digitalworld' )
		);
		...
		parent::__construct( 'digitalworld_instagram', esc_html__( 'Digitalworld: Instagram', 'digitalworld' ), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$output = '<div class="digitalworld-instagram clear cols-' . esc_attr( $columns ) . '">';
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'id' ) ); ?>"><?php echo sprintf( esc_html__( 'Instagram user ID (<a href="%1$s" target="_blank">Lookup your User ID</a>)', 'digitalworld' ), $lookup_url ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'token' ) ); ?>"><?php echo sprintf( esc_html__( 'Access token (<a href="%1$s" target="_blank">Generate access token</a>)', 'digitalworld' ), $generate_token ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'limit' ) ); ?>"><?php esc_html_e( 'Number of Photos:', 'digitalworld' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'columns' ) ); ?>"><?php esc_html_e( 'Columns (1-5):', 'digitalworld' ); ?></label>
...
add_action( 'widgets_init', 'Digitalworld_Widgets_Instagram' );
function Digitalworld_Widgets_Instagram() {
	register_widget( 'Digitalworld_Widgets_Instagram' );
}
